...
clean_arg <- function(my_arg, c=" ")
{
    if (nchar(my_arg) > 1 && substr(my_arg, start = nchar(my_arg), stop = nchar(my_arg)) == c) {
        return (clean_arg(substr(my_arg, start = 1, stop = nchar(my_arg) - 1), c))
    } else {
        return (my_arg)
    }
}
...
#Path pour VSoil:
path_dir_vsoil <- clean_arg(toString(args[1]), "/")
...
#Path pour ESPAS:
path_dir_espas <- clean_arg(toString(args[2]), "/")
...
compare_files <- function(path_file1, path_file2, epsilon, sep)
{
    if (script_error == FALSE) {
        ...
        if (script_error == FALSE) {
            cat("\\textcolor{green}{Les deux fichiers ont le même nombre de lignes.} \\\n", sep="")
        }
        compare_data(data1, data2, epsilon, path_file1, path_file2)
        if (script_error == FALSE) {
            cat("\\textcolor{green}{Les deux fichiers ont des valeurs identiques.} \\\n", sep="")
        } else {
            return (FALSE)
        }
    }
