...
compare_data <- function(data1, data2, epsilon, path_file1, path_file2)
{
    if (script_error == FALSE) {
        ...
        if (is.element(TRUE, cmp_data)) {
            ...
            cat("\\textcolor{red}{Valeur: ", unlist(data1[error_colone])[error_line], " est invalide, LIGNE: ", error_line, ", COLONE: ",error_colone, "} \\\n", sep="")
        }
    }
}
...
compare_files_lines <- function(path_file1, path_file2, epsilon)
{
    ...
    #   Vérification des valeurs dans les deux fichiers:
        check_epsilon(data1[11], data2[11], epsilon, path_file1, path_file2) #Diametre interieur du cylindre
        check_epsilon(data1[14], data2[14], epsilon, path_file1, path_file2) #Hauteur du cylindre
        check_epsilon(data1[17], data2[17], epsilon, path_file1, path_file2) #Masse de la tare utilisée
        check_epsilon(data1[20], data2[20], epsilon, path_file1, path_file2) #Masse totale de l'echantillon humide
        check_epsilon(data1[23], data2[23], epsilon, path_file1, path_file2) #Masse nette de sol sec a 105 degre
        check_epsilon(data1[26], data2[26], epsilon, path_file1, path_file2) #Masse de la premiere sequence
        check_epsilon(data1[29], data2[29], epsilon, path_file1, path_file2) #Hauteur totale du systeme experimental
        if (script_error == FALSE) {
            cat("\\textcolor{green}{Les deux fichiers ont des valeurs identiques: } (", path_file1, " & ", path_file2, ") \\\n", sep="")
        }
    }
...
```{r, echo = FALSE, fig.width=9, fig.height=8, results="asis", linewidth=90}
    display_path("Chemin des données du .ter", path_ter_vsoil, path_ter_espas)
...
```{r, echo = FALSE, fig.width=9, fig.height=8, results="asis", linewidth=90}
    ...
    if (!compare_files(path_calibration_vsoil, path_calibration_espas, 0.001, "\t")) {
        ...
        data_2$My_time <- unlist(data_2[1]) / 3600
